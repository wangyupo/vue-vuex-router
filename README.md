...
```
1、用Vue做一个展示网站。网站按页面划分模块，每个页面按section（部分）再划分模块。培养自己的模块化思想。
2、用Vue+Axios做一个带请求的网站。把请求结果放在页面上展示出来。锻炼请求接口的能力，了解前后端分离思想。
3、用Vue+Axios+Vuex做一个能管理数据的网站。把请求放到Vuex中，用store管理数据。搞懂Vuex在开发中的帮助，提升项目开发能力。

做完上面三步你就已经入门Vue了，剩下的就是用这个最佳实践来构建更优雅的代码、组织更简洁的项目。
```
